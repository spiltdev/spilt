...
## Quick start: route through the gateway

The fastest path — send an inference request through Pura in two lines. No wallet needed, just an API key.
...
Or skip the SDK entirely — the gateway is OpenAI-compatible:
...
Read live on-chain state with one call:

```typescript
import { createPublicClient, http } from 'viem'
import { baseSepolia } from 'viem/chains'
import { monitor, getAddresses } from '@puraxyz/sdk'

const client = createPublicClient({ chain: baseSepolia, transport: http() })
const state = await monitor(client, getAddresses(84532))

console.log(state.sinkCount)       // registered providers
console.log(state.poolTotalUnits)  // total capacity units
console.log(state.baseFee)         // current base fee
```

## Deploy as a provider

Register capacity, stake tokens, and join the pool in one call:

```typescript
import { deploy, getAddresses } from '@puraxyz/sdk'

const txs = await deploy(wallet, getAddresses(84532), {
  taskTypeId: '0x' + Buffer.from('text-generation').toString('hex').padEnd(64, '0') as `0x${string}`,
  initialCapacity: 100n,
  stakeAmount: 400n * 10n ** 18n,
})

console.log('Registered:', txs.registerTx)
console.log('Staked:', txs.stakeTx)
console.log('Connected to pool:', txs.connectTx)
```

## Full setup (advanced)

The rest of this guide covers the lower-level SDK modules for full control.
