...
import { generateLightningSeedState, type NodeInfo } from "@/lib/shared/seed";
...
function hasData(data: LightningState) {
  return data.nodes && data.nodes.length > 0;
}

export default function LightningPage() {
  const { state, loading } = usePolledState<LightningState>(
    "/api/lightning/state",
    hasData,
  ...
  async function handleFindRoute(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/lightning/route?${params}`);
      if (res.ok) {
