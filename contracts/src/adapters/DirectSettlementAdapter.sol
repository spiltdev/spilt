...
/// @title DirectSettlementAdapter
/// @notice Simple ERC-20 transfer-based settlement. No streaming, no HTLCs — just
///         direct transfers and a basic escrow mapping. Useful for one-shot DVM jobs
///         where streaming overhead is unnecessary.
contract DirectSettlementAdapter is ISettlementAdapter {
    using SafeERC20 for IERC20;

    // ──────────────────── Constants ────────────────────

    bytes32 public constant RAIL_ID = keccak256("DIRECT");
    ...
    function adjustStream(bytes32 streamId, uint256 newFlowRate) external onlyPipeline {
        ...
        // Settle accrued before adjusting
        uint256 accrued = _accruedUnsettled(s);
        if (accrued > 0) {
            s.settled += accrued;
    ...
    function _accruedUnsettled(Stream storage s) internal view returns (uint256) {
